...
exports.default = async function (tokens, message) {
    ...
    if (tokens.length < 2) return message.channel.send('', {
        embed: {
            title: 'Help: Era',
            description: 'To use this command, please supply an era and a BattleTag\nFor example, `leah era 8 user-1234`',
            color: 0xFF33A2
        }
    });

    var era = tokens.shift();
    var battleTag = tokens.shift().replace('#', '-');
    var region = await getRegion(tokens.shift(), message);

    var leaderboards = await _profile2.default.getEra(region, era);
    ...
    var leaderboard = await _profile2.default.getEraLeaderboard(region, era, leaderboards[choice - 1]);
    var rank = void 0;
